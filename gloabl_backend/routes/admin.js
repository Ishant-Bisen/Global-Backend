...
const transModel = require("../database/schema/TransictionDetails")
...
route.post("/user/amount/allocation", verifyJWT, async (req, res) => {
    try {
        ...
        if(allocated_amount != undefined){
            const data = await transModel.create({
                uid:user.uid,
                user:user.account_holder_name,
            ...
            if(!data) return res.status(201).send({message : "Allocated amount is successfully credited but transaction histroy is not created."})
        }
